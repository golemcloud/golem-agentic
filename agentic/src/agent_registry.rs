...
static AGENT_DEF_REGISTRY: Lazy<Mutex<HashMap<AgentTraitName, AgentDefinition>>> = Lazy::new(|| Mutex::new(HashMap::new()));
...
pub fn register_agent_definition(agent_trait_name: String, def: AgentDefinition) {
    AGENT_DEF_REGISTRY.lock().unwrap().insert(agent_trait_name, def);
}
...
pub fn get_agent_def_by_name(agent_trait_name: &str) -> Option<AgentDefinition> {
    AGENT_DEF_REGISTRY.lock().unwrap().get(agent_trait_name).cloned()
}

pub fn get_all_agent_definitions() -> Vec<AgentDefinition> {
    AGENT_DEF_REGISTRY.lock().unwrap().values().cloned().collect::<Vec<_>>()
}
